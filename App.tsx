...
// ... existing SingleFundPerformanceCard, AddAssetModal, LiquidityRuleModal, PatchConfigModal components ...
// (Retaining these imports but focusing changes below)

// ... [Existing Components: SingleFundPerformanceCard, AddAssetModal, LiquidityRuleModal, PatchConfigModal] ...

// --- Simplified Component Declarations for Context (Body content same as input file) ---
const SingleFundPerformanceCard: React.FC<{fund: Fund; patchRules: PatchRule[]; allFunds: Fund[];}> = ({ fund, patchRules, allFunds }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col h-[280px]">
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="flex bg-gray-100 rounded-md p-0.5 shrink-0">
          {COMPACT_TIME_RANGES.map(r => (
            <button key={r.label} onClick={() => setRange(r.value)} className={`px-1.5 py-0.5 text-[10px] font-medium rounded-sm transition-all ${range === r.value ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{r.label}</button>
          ))}
        </div>
      </div>
      <div className="flex-1 min-h-0 relative">
         <ComparisonChart data={chartDataObj.chartData} funds={[fund]} patchRules={patchRules} allFunds={allFunds} metric="NAV" viewMode="OVERLAY" compact={true} hideTitle={true} />
      </div>
      <div className="mt-3 flex items-center justify-between text-xs pt-3 border-t border-gray-50 border-dashed">
         ...
         <span className={`font-medium ${fund.dayChange >= 0 ? "text-red-500" : "text-green-500"}`}>{fund.dayChange >= 0 ? '+' : ''}{fund.dayChange}%</span>
      </div>
    </div>
  );
};
...
const LiquidityRuleModal: React.FC<{isOpen: boolean; onClose: () => void; holdingName: string; currentRule?: RedemptionRule; onSave: (rule: RedemptionRule) => void;}> = (props) => {
    ...
    useEffect(() => {
        if (isOpen) {
            setRuleType(currentRule?.ruleType || 'DAILY'); setOpenDay(currentRule?.openDay || 15); setSettlementDays(currentRule?.settlementDays || 3);
            if (currentRule?.lockupEndDate) { setHasLockup(true); setLockupEndDate(currentRule.lockupEndDate); } else { setHasLockup(false); setLockupEndDate(''); }
            if (currentRule?.maturityDate) { setMaturityDate(currentRule.maturityDate); } else { setMaturityDate(''); }
        }
    }, [isOpen, currentRule]);
    if (!isOpen) return null;
    const handleSave = () => { onSave({ ruleType, openDay: ruleType === 'MONTHLY' ? openDay : undefined, settlementDays, lockupEndDate: (hasLockup && ruleType !== 'FIXED_TERM') ? lockupEndDate : undefined, maturityDate: ruleType === 'FIXED_TERM' ? maturityDate : undefined }); onClose(); };
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
                <div className="flex justify-between items-center mb-6"> <h3 className="text-lg font-bold text-gray-900">配置流动性规则</h3> <button onClick={onClose}><X className="w-5 h-5 text-gray-500" /></button> </div>
                <p className="text-xs text-gray-500 mb-4 bg-gray-50 p-2 rounded">资产：<span className="font-medium text-gray-900">{holdingName}</span></p>
                <div className="space-y-4">
                    <div> <label className="block text-xs font-medium text-gray-700 mb-1">开放类型</label> <div className="flex flex-col gap-1 bg-gray-100 p-1 rounded-lg"> <div className="flex gap-1"> <button onClick={() => setRuleType('DAILY')} className={`flex-1 text-xs py-1.5 font-medium rounded-md transition-all ${ruleType === 'DAILY' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}>每日开放 (标准)</button> <button onClick={() => setRuleType('MONTHLY')} className={`flex-1 text-xs py-1.5 font-medium rounded-md transition-all ${ruleType === 'MONTHLY' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}>定期开放 (信托)</button> </div> <button onClick={() => setRuleType('FIXED_TERM')} className={`w-full text-xs py-1.5 font-medium rounded-md transition-all ${ruleType === 'FIXED_TERM' ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}>类信托/固收 (到期自动赎回)</button> </div> </div>
                    {ruleType === 'MONTHLY' && ( <div> <label className="block text-xs font-medium text-gray-700 mb-1">每月开放日</label> <div className="flex items-center gap-2"> <span className="text-sm text-gray-500">每月</span> <input type="number" min={1} max={31} value={openDay} onChange={e => setOpenDay(parseInt(e.target.value))} className="w-20 text-sm border-gray-300 rounded-md"/> <span className="text-sm text-gray-500">日</span> </div> </div> )}
                    {ruleType === 'FIXED_TERM' && ( <div> <label className="block text-xs font-medium text-gray-700 mb-1">到期/终止日</label> <input type="date" value={maturityDate} onChange={e => setMaturityDate(e.target.value)} className="w-full text-sm border-gray-300 rounded-md"/> <p className="text-[10px] text-gray-500 mt-1">资金将在到期后自动赎回，无需手动操作。</p> </div> )}
...
const PatchConfigModal: React.FC<{isOpen: boolean; onClose: () => void; patchRules: PatchRule[]; onAddRule: (rule: PatchRule) => void; onRemoveRule: (id: string) => void; allFunds: Fund[]; comparisonStartDate: string;}> = (props) => {
    // ... existing implementation
    const { isOpen, onClose, patchRules, onAddRule, onRemoveRule, allFunds, comparisonStartDate } = props;
    const [targetId, setTargetId] = useState(''); const [proxyId, setProxyId] = useState(''); const [startDate, setStartDate] = useState(''); const [endDate, setEndDate] = useState('');
    const gapInfo = useMemo(() => { if (!targetId || !comparisonStartDate) return null; const fund = allFunds.find(f => f.id === targetId); if (!fund) return null; if (fund.inceptionDate > comparisonStartDate) { const inception = new Date(fund.inceptionDate); inception.setDate(inception.getDate() - 1); const end = inception.toISOString().split('T')[0]; return { hasGap: true, gapStart: comparisonStartDate, gapEnd: end, fundInception: fund.inceptionDate }; } return { hasGap: false }; }, [targetId, comparisonStartDate, allFunds]);
    if (!isOpen) return null;
    const handleAdd = () => { if (targetId && proxyId && startDate && endDate) { onAddRule({ id: Date.now().toString(), targetFundId: targetId, proxyFundId: proxyId, startDate, endDate }); setProxyId(''); setStartDate(''); setEndDate(''); } };
    const autoFillGap = () => { if (gapInfo?.hasGap) { setStartDate(gapInfo.gapStart); setEndDate(gapInfo.gapEnd); } };
    const getFundName = (id: string) => allFunds.find(f => f.id === id)?.name || id;
    const visStartDate = new Date(comparisonStartDate); const visEndDate = new Date(); const visGapStart = gapInfo?.hasGap ? new Date(gapInfo.gapStart) : undefined; const visGapEnd = gapInfo?.hasGap ? new Date(gapInfo.gapEnd) : undefined;
    const relevantRules = patchRules.filter(r => r.targetFundId === targetId).map(r => ({ start: new Date(r.startDate), end: new Date(r.endDate) }));
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center"> <div> <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2"> <Wand2 className="w-5 h-5 text-indigo-600" /> 配置净值补齐 </h3> <p className="text-sm text-gray-500 mt-1">为成立时间较短的基金补充历史业绩数据</p> </div> <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full transition-colors"> <X className="w-5 h-5 text-gray-500" /> </button> </div>
                <div className="p-6 overflow-y-auto flex-1">
                  <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4"> <div> <label className="block text-xs font-medium text-gray-700 mb-1">目标基金 (需补齐)</label> <select className="w-full text-sm border-gray-300 rounded-md" value={targetId} onChange={(e) => setTargetId(e.target.value)}> <option value="">选择基金...</option> {allFunds.map(f => ( <option key={f.id} value={f.id}>{f.name} (成立: {f.inceptionDate})</option> ))} </select> </div> <div> <label className="block text-xs font-medium text-gray-700 mb-1">替补基金 (数据源)</label> <select className="w-full text-sm border-gray-300 rounded-md" value={proxyId} onChange={(e) => setProxyId(e.target.value)}> <option value="">选择相似基金...</option> {allFunds.filter(f => f.id !== targetId).map(f => ( <option key={f.id} value={f.id}>{f.name}</option> ))} </select> </div> <div> <label className="block text-xs font-medium text-gray-700 mb-1">开始日期</label> <input type="date" className="w-full text-sm border-gray-300 rounded-md" value={startDate} onChange={(e) => setStartDate(e.target.value)} /> </div> <div> <label className="block text-xs font-medium text-gray-700 mb-1">结束日期</label> <input type="date" className="w-full text-sm border-gray-300 rounded-md" value={endDate} onChange={(e) => setEndDate(e.target.value)} /> </div> </div>
                    {gapInfo?.hasGap && ( <div className="mb-4 bg-orange-50 border border-orange-200 rounded-lg p-3 flex items-start justify-between"> <div className="flex gap-2"> <AlertTriangle className="w-4 h-4 text-orange-600 mt-0.5" /> <div className="text-xs text-orange-800"> <span className="font-bold">发现数据缺口:</span> 建议补齐 <span className="font-mono">{gapInfo.gapStart} ~ {gapInfo.gapEnd}</span> </div> </div> <button onClick={autoFillGap} className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded hover:bg-orange-200 font-medium">一键填充</button> </div> )}
                    {targetId && ( <div className="mb-4"> <label className="block text-xs font-medium text-gray-500 mb-1">补齐覆盖预览</label> <TimelineVisualizer startDate={visStartDate} endDate={visEndDate} gapStart={visGapStart} gapEnd={visGapEnd} patchRules={relevantRules} /> </div> )}
                  ...
                  <div>
                    ...
                    {patchRules.length === 0 ? <div className="text-center py-8 text-gray-400 bg-gray-50 rounded-lg border border-dashed border-gray-200 text-sm">暂无补齐规则</div> : <div className="space-y-3"> {patchRules.map(rule => ( <div key={rule.id} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg shadow-sm"> <div className="flex flex-col gap-1"> <div className="flex items-center gap-2 text-sm text-gray-900"> <span className="font-medium">{getFundName(rule.targetFundId)}</span> <ArrowLeft className="w-3 h-3 text-gray-400" /> <span className="text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded text-xs border border-indigo-100">{getFundName(rule.proxyFundId)}</span> </div> <div className="text-xs text-gray-500 font-mono">{rule.startDate} 至 {rule.endDate}</div> </div> <button onClick={() => onRemoveRule(rule.id)} className="text-gray-400 hover:text-red-500 p-2 hover:bg-red-50 rounded-full"><Trash2 className="w-4 h-4" /></button> </div> ))} </div> }
                  </div>
                </div>
                <div className="p-4 border-t border-gray-100 flex justify-end"> <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-md">完成配置</button> </div>
            </div>
        </div>
    );
};
...
const LiquidityRulesCard: React.FC = () => (
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                <div className="flex flex-col gap-2"> <div className="flex items-start gap-2"> <span className="bg-blue-100 text-blue-700 px-1.5 rounded text-xs font-medium whitespace-nowrap mt-0.5">现金/活期</span> <span>T+0 实时可用，直接计入可用资金。</span> </div> <div className="flex items-start gap-2"> <span className="bg-green-100 text-green-700 px-1.5 rounded text-xs font-medium whitespace-nowrap mt-0.5">标准资产</span> <span>每日开放赎回，在 T+N 结算期结束后计入可用资金。</span> </div> <div className="flex items-start gap-2"> <span className="bg-amber-100 text-amber-700 px-1.5 rounded text-xs font-medium whitespace-nowrap mt-0.5">定期开放</span> <span> 非开放期锁定。需在"现金流规划"中手动录入赎回计划，资金将于<span className="font-mono text-gray-800">计划日期 + 结算期</span>后计入可用。 </span> </div> </div>
                <div className="flex flex-col gap-2"> <div className="flex items-start gap-2"> <span className="bg-purple-100 text-purple-700 px-1.5 rounded text-xs font-medium whitespace-nowrap mt-0.5">固定期限</span> <span>到期日自动赎回，资金于<span className="font-mono text-gray-800">到期日 + 结算期</span>后自动计入可用，无需手动操作。</span> </div> <div className="flex items-start gap-2"> <span className="bg-red-100 text-red-700 px-1.5 rounded text-xs font-medium whitespace-nowrap mt-0.5">锁定期</span> <span>资产在锁定期截止前不可赎回，解禁后按其开放规则执行。</span> </div> <div className="flex items-start gap-2"> <span className="bg-gray-100 text-gray-700 px-1.5 rounded text-xs font-medium whitespace-nowrap mt-0.5">赎回结算</span> <span>所有赎回操作均考虑 T+N 资金在途时间，确保预测真实到账日期。</span> </div> </div>
            </div>
        </div>
);

// ... existing Page Components (FundListPage, ComparisonPage, FundDetailPage, PortfolioPage, LiquidityPage) ...
// (These are kept essentially the same, just ensuring imports work)
...
const FundListPage: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center"> <div> <h1 className="text-2xl font-bold text-gray-900">基金产品列表</h1> <p className="mt-1 text-sm text-gray-500">A股市场规模领先的ETF基金，点击名称查看详情，或选择多只进行对比。</p> </div> <div className="flex gap-3"> <div className="bg-white px-3 py-2 rounded-lg border border-gray-200 flex items-center text-sm text-gray-500 shadow-sm"> <span className="font-medium text-indigo-600 mr-1">{selectedFunds.size}</span> 已选中 </div> <button onClick={() => navigate('/comparison', { state: { selectedIds: Array.from(selectedFunds) } })} disabled={selectedFunds.size < 1} className="inline-flex items-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"> <BarChart2 className="w-4 h-4 mr-2" /> 开始对比 </button> </div> </div>
            <div className="bg-white shadow-sm rounded-xl overflow-hidden border border-gray-200">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50"> <tr> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-10">选择</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">基金代码/名称</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">类型</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">成立日期</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">最新净值</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">日涨跌幅</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">今年以来</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">操作</th> </tr> </thead>
                    <tbody className="bg-white divide-y divide-gray-200"> {MOCK_FUNDS.map((fund) => ( <tr key={fund.id} onClick={() => toggleFund(fund.id)} className={`hover:bg-gray-50 transition-colors cursor-pointer ${selectedFunds.has(fund.id) ? 'bg-indigo-50/30' : ''}`}> <td className="px-6 py-4 whitespace-nowrap"> <input type="checkbox" checked={selectedFunds.has(fund.id)} onClick={(e) => e.stopPropagation()} onChange={() => toggleFund(fund.id)} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded cursor-pointer"/> </td> <td className="px-6 py-4 whitespace-nowrap"> <div className="flex flex-col"> <Link to={`/fund/${fund.id}`} onClick={(e) => e.stopPropagation()} className="text-sm font-medium text-gray-900 hover:text-indigo-600 hover:underline">{fund.name}</Link> <span className="text-xs text-gray-500 font-mono">{fund.code}</span> </div> </td> <td className="px-6 py-4 whitespace-nowrap"><Badge color={getFundTypeColor(fund.type)}>{fund.type}</Badge></td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{fund.inceptionDate}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-mono">{fund.nav.toFixed(4)}</td> <td className="px-6 py-4 whitespace-nowrap text-sm"><span className={`font-medium ${fund.dayChange >= 0 ? 'text-red-600' : 'text-green-600'}`}>{fund.dayChange > 0 ? '+' : ''}{fund.dayChange}%</span></td> <td className="px-6 py-4 whitespace-nowrap text-sm"><span className={`font-medium ${fund.ytdReturn >= 0 ? 'text-red-600' : 'text-green-600'}`}>{fund.ytdReturn > 0 ? '+' : ''}{fund.ytdReturn}%</span></td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500"> <Link to={`/fund/${fund.id}`} onClick={(e) => e.stopPropagation()} className="text-indigo-600 hover:text-indigo-900 flex items-center gap-1"><Search className="w-3 h-3"/> 详情</Link> </td> </tr> ))} </tbody>
                </table>
            </div>
        </div>
    );
...
const ComparisonPage: React.FC<{ patchRules: PatchRule[], onAddPatchRule: (r: PatchRule) => void, onRemovePatchRule: (id: string) => void }> = ({ patchRules, onAddPatchRule, onRemovePatchRule }) => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center"> <h1 className="text-2xl font-bold text-gray-900">业绩对比分析</h1> <div className="flex gap-2"> <button onClick={() => setIsPatchModalOpen(true)} className="flex items-center px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 shadow-sm text-sm font-medium"> <Wand2 className="w-4 h-4 mr-2 text-indigo-600" /> 配置净值补齐 </button> <button onClick={handleAnalyze} disabled={analysis.loading || funds.length === 0} className="flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-sm text-sm font-medium disabled:opacity-50"> {analysis.loading ? <span className="animate-spin mr-2">⏳</span> : <Sparkles className="w-4 h-4 mr-2" />} AI 智能分析 </button> </div> </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm"> <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4"> <Calculator className="w-5 h-5 text-indigo-600" /> 归一化净值说明 </h3> <div className="prose prose-sm text-gray-600"> <p>为了直观对比不同基金的业绩走势，我们将所有基金在<strong>统计起始日</strong>的净值统一换算为 <strong>100</strong>。</p> <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 font-mono text-xs my-3"> <div className="mb-1 text-gray-500">// 计算公式</div> 归一化净值(t) = ( 实际净值(t) / 起始日实际净值 ) × 100 </div> <p> 例如：若某基金起始日净值为 1.50，当前净值为 1.65，则：<br/> 归一化净值 = (1.65 / 1.50) × 100 = <strong>110.00</strong><br/> 这表示该区间内上涨了 10%。 </p> </div> </div> <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm"> <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4"> <Sparkles className="w-5 h-5 text-indigo-600" /> AI 分析报告 </h3> <div className="bg-indigo-50/50 rounded-lg p-4 min-h-[160px]"> {analysis.loading && <div className="text-gray-500 text-sm animate-pulse">正在生成分析报告...</div>} {!analysis.loading && !analysis.content && <div className="text-gray-400 text-sm">点击右上角“AI 智能分析”获取报告</div>} {analysis.content && <div className="prose prose-sm text-gray-800"><ReactMarkdown>{analysis.content}</ReactMarkdown></div>} </div> </div> </div>
            <PatchConfigModal isOpen={isPatchModalOpen} onClose={() => setIsPatchModalOpen(false)} patchRules={patchRules} onAddRule={onAddPatchRule} onRemoveRule={onRemovePatchRule} allFunds={MOCK_FUNDS} comparisonStartDate={new Date(new Date().setDate(new Date().getDate() - (typeof selectedRange === 'number' ? selectedRange : 365))).toISOString().split('T')[0]} />
        </div>
    );
};
const FundDetailPage: React.FC<{ patchRules: PatchRule[], onAddPatchRule: (r: PatchRule) => void, onRemovePatchRule: (id: string) => void }> = ({ patchRules, onAddPatchRule, onRemovePatchRule }) => {
    ...
    const filteredData = useMemo(() => { if (filterPatched) { return historyData.filter(d => d.isPatched); } return historyData; }, [historyData, filterPatched]);
    useEffect(() => setCurrentPage(1), [filterPatched, range]);
    const paginatedData = useMemo(() => { const reversed = [...filteredData].reverse(); const start = (currentPage - 1) * PAGE_SIZE; return reversed.slice(start, start + PAGE_SIZE); }, [filteredData, currentPage]);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden"> <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50"> <h3 className="font-bold text-gray-900">历史净值明细</h3> <div className="flex items-center gap-2"> <Filter className="w-4 h-4 text-gray-400"/> <label className="text-sm text-gray-600 flex items-center gap-2 cursor-pointer select-none"> <input type="checkbox" checked={filterPatched} onChange={e => setFilterPatched(e.target.checked)} className="rounded text-indigo-600 focus:ring-indigo-500"/> 只看补齐数据 </label> </div> </div> <table className="min-w-full divide-y divide-gray-200"> <thead className="bg-gray-50"> <tr> <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">日期</th> <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">单位净值</th> <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">日涨跌幅</th> <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">数据来源</th> </tr> </thead> <tbody className="bg-white divide-y divide-gray-200"> {paginatedData.map((row, idx) => ( <tr key={idx} className="hover:bg-gray-50"> <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-900 font-mono">{row.date}</td> <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-900 font-mono"> {(row.nav_actual || row.nav_patched)?.toFixed(4)} </td> <td className="px-6 py-3 whitespace-nowrap text-sm"> <span className={`font-medium ${(row.change || 0) >= 0 ? 'text-red-600' : 'text-green-600'}`}> {(row.change || 0) > 0 ? '+' : ''}{row.change}% </span> </td> <td className="px-6 py-3 whitespace-nowrap text-sm"> {row.isPatched ? ( <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200"> <Wand2 className="w-3 h-3"/> 补齐: {row.proxyName} </span> ) : ( <span className="text-gray-500">真实数据</span> )} </td> </tr> ))} </tbody> </table> <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6"> <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between"> <div> <p className="text-sm text-gray-700"> 显示 <span className="font-medium">{(currentPage - 1) * PAGE_SIZE + 1}</span> 到 <span className="font-medium">{Math.min(currentPage * PAGE_SIZE, filteredData.length)}</span> 条，共 <span className="font-medium">{filteredData.length}</span> 条 </p> </div> <div> <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination"> <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50" > <ChevronLeft className="h-5 w-5" /> </button> <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50" > <ChevronRight className="h-5 w-5" /> </button> </nav> </div> </div> </div> </div>
            <PatchConfigModal isOpen={isPatchModalOpen} onClose={() => setIsPatchModalOpen(false)} patchRules={patchRules} onAddRule={onAddPatchRule} onRemoveRule={onRemovePatchRule} allFunds={MOCK_FUNDS} comparisonStartDate={new Date(new Date().setDate(new Date().getDate() - (typeof range === 'number' ? range : 365))).toISOString().split('T')[0]} />
        </div>
    );
};
const PortfolioPage: React.FC<{ portfolio: ClientPortfolio, patchRules: PatchRule[], onAddExternalAsset: (accountId: string, holding: Holding) => void }> = ({ portfolio, patchRules, onAddExternalAsset }) => {
  ...
  const totalAssets = portfolio.accounts.reduce((sum, acc) => sum + (acc.cashBalance || 0) + acc.holdings.reduce((hSum, h) => hSum + getHoldingValue(h), 0), 0);
  const getAssetAllocation = (holdings: Holding[], cash: number) => { const allocation: {[key: string]: number} = { 'CASH': cash }; holdings.forEach(h => { const type = getHoldingType(h); const val = getHoldingValue(h); allocation[type] = (allocation[type] || 0) + val; }); return Object.entries(allocation).map(([name, value]) => ({ name, value })); };
  const totalAllocation = useMemo(() => { const allHoldings = portfolio.accounts.flatMap(a => a.holdings); const totalCash = portfolio.accounts.reduce((s, a) => s + (a.cashBalance || 0), 0); return getAssetAllocation(allHoldings, totalCash); }, [portfolio]);
  const uniqueFundIds = useMemo(() => { const funds = new Set<string>(); const accountsToScan = selectedAccountId === 'ALL' ? portfolio.accounts : portfolio.accounts.filter(a => a.id === selectedAccountId); accountsToScan.forEach(acc => { acc.holdings.forEach(h => { if (!h.isExternal && h.fundId) funds.add(h.fundId); }); }); return Array.from(funds); }, [portfolio, selectedAccountId]);
  const displayedFunds = MOCK_FUNDS.filter(f => uniqueFundIds.includes(f.id));
  const performanceChartDataObj = useMemo(() => { if (chartView === 'GRID') return { chartData: [], gaps: [] }; let days = 180; if (typeof selectedRange === 'number') days = selectedRange; else if (selectedRange === 'YTD') days = 365; return generateChartData(displayedFunds, days, patchRules, MOCK_FUNDS); }, [displayedFunds, selectedRange, patchRules, chartView]);
...
const LiquidityPage: React.FC<{ portfolio: ClientPortfolio, updateHoldingRule: (accId: string, holdingIdx: number, rule: RedemptionRule) => void, updateAccountCash: (accId: string, amount: number) => void }> = ({ portfolio, updateHoldingRule, updateAccountCash }) => {
  ...
  const projectionData = useMemo(() => { const days = 30; const data = []; const today = new Date(); today.setHours(0,0,0,0); let currentCash = liquidityData[LiquidityTier.CASH]; const redeemedAmounts = new Map<string, number>(); const sortedFlows = [...cashFlows].sort((a,b) => a.date.localeCompare(b.date)); const holdingsWithArrival = currentAccountHoldings.map(h => { let val = 0; let type = FundType.STRATEGY; if (h.isExternal) { val = (h.externalNav || 0) * h.shares; type = h.externalType || FundType.STRATEGY; } else { const f = MOCK_FUNDS.find(fund => fund.id === h.fundId); if (f) { val = f.nav * h.shares; type = f.type; } } const arrivalDate = calculateAvailabilityDate(today, h, type); arrivalDate.setHours(0,0,0,0); return { ...h, value: val, arrivalDate: arrivalDate, uniqueKey: h.uniqueKey }; }); for (let i = 0; i < days; i++) { const date = new Date(today); date.setDate(today.getDate() + i); date.setHours(0,0,0,0); const dateStr = date.toISOString().split('T')[0]; const displayDate = `${date.getMonth()+1}-${date.getDate()}`; const dailyFlows = sortedFlows.filter(f => f.date === dateStr); const inflow = dailyFlows.filter(f => f.type === 'INFLOW').reduce((sum, f) => sum + f.amount, 0); const outflow = dailyFlows.filter(f => f.type === 'OUTFLOW').reduce((sum, f) => sum + f.amount, 0); dailyFlows.forEach(f => { if (f.relatedHoldingKey && f.type === 'INFLOW') { const currentRedeemed = redeemedAmounts.get(f.relatedHoldingKey) || 0; redeemedAmounts.set(f.relatedHoldingKey, currentRedeemed + f.amount); } }); currentCash = currentCash + inflow - outflow; let liquidAssetsFromHoldings = 0; let lockedAssets = 0; const lockedDetailsList: { name: string; value: number; reason: string }[] = []; holdingsWithArrival.forEach(h => { const redeemed = redeemedAmounts.get(h.uniqueKey) || 0; const remainingVal = Math.max(0, h.value - redeemed); if (remainingVal > 0) { const isPeriodic = h.redemptionRule?.ruleType === 'MONTHLY'; const hasLockup = !!h.redemptionRule?.lockupEndDate; const isFixedTerm = h.redemptionRule?.ruleType === 'FIXED_TERM'; if (!isPeriodic && date.getTime() >= h.arrivalDate.getTime()) { liquidAssetsFromHoldings += remainingVal; } else { lockedAssets += remainingVal; let reason = ""; const diffTime = h.arrivalDate.getTime() - date.getTime(); const daysUntilArrival = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); if (isFixedTerm && h.redemptionRule?.maturityDate) { reason = `持有至到期 (到期日: ${h.redemptionRule.maturityDate})`; } else if (hasLockup) { const lockupEnd = new Date(h.redemptionRule!.lockupEndDate!); lockupEnd.setHours(0,0,0,0); if (date.getTime() < lockupEnd.getTime()) { reason = `处于锁定期 (至 ${h.redemptionRule!.lockupEndDate})`; } } if (!reason) { if (isPeriodic) { const settlementDays = h.redemptionRule?.settlementDays || 0; const openDate = new Date(h.arrivalDate); openDate.setDate(openDate.getDate() - settlementDays); if (date.getTime() < openDate.getTime()) { reason = `非开放期 (每月${h.redemptionRule?.openDay}日)`; } else { reason = `赎回结算中 (T+${Math.max(0, daysUntilArrival)})`; } } else { reason = `赎回结算中 (T+${Math.max(0, daysUntilArrival)})`; } } lockedDetailsList.push({ name: h.displayName || '未知资产', value: remainingVal, reason: reason }); } } }); lockedDetailsList.sort((a, b) => b.value - a.value); data.push({ date: dateStr, displayDate: displayDate, liquid: currentCash + liquidAssetsFromHoldings, locked: lockedAssets, expense: -outflow, rawExpense: outflow, lockedBreakdown: lockedDetailsList }); } return data; }, [portfolio, cashFlows, liquidityData, selectedAccountId, currentAccountHoldings]);
  ...
  const CustomChartTooltip = ({ active, payload, label }: any) => { if (active && payload && payload.length) { const data = payload[0].payload; const liquid = data.liquid; const locked = data.locked; const expense = Math.abs(data.rawExpense); const lockedBreakdown = data.lockedBreakdown || []; return ( <div className="bg-white rounded-xl shadow-xl border border-gray-100 p-4 min-w-[280px] text-sm"> <div className="flex items-center justify-between mb-3 pb-2 border-b border-gray-50"> <span className="font-bold text-gray-900">{label} (日期)</span> <span className="text-xs text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">预测详情</span> </div> <div className="space-y-3"> <div className="flex justify-between items-center"> <span className="text-gray-500 flex items-center gap-1"><div className="w-2 h-2 bg-indigo-600 rounded-full"></div>可用流动性</span> <span className="font-mono font-bold text-gray-900">¥ {liquid.toLocaleString()}</span> </div> <div className="flex flex-col gap-1"> <div className="flex justify-between items-center"> <span className="text-gray-500 flex items-center gap-1"><div className="w-2 h-2 bg-slate-300 rounded-full"></div>锁定流动性</span> <span className="font-mono font-medium text-gray-600">¥ {locked.toLocaleString()}</span> </div> {lockedBreakdown.length > 0 && ( <div className="bg-gray-50 rounded p-2 mt-1 space-y-1 max-h-[150px] overflow-y-auto custom-scrollbar border border-gray-100"> {lockedBreakdown.map((item: any, idx: number) => ( <div key={idx} className="flex justify-between items-start text-xs"> <span className="text-gray-600 truncate max-w-[120px]" title={item.name}>{item.name}</span> <div className="text-right"> <div className="font-mono text-gray-700">¥{(item.value/10000).toFixed(1)}万</div> <div className="text-[10px] text-gray-400 scale-90 origin-right">{item.reason}</div> </div> </div> ))} </div> )} </div> {expense > 0 && ( <div className="flex justify-between items-center pt-2 border-t border-gray-50 text-red-600"> <span className="flex items-center gap-1"><div className="w-2 h-2 bg-red-500 rounded-full"></div>当日支出</span> <span className="font-mono font-bold">- ¥ {expense.toLocaleString()}</span> </div> )} </div> </div> ); } return null; };
  return (
