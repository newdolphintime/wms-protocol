...
const FundListPage: React.FC = () => {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">正在加载基金列表...</div>;
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // If we need patch logic (frontend simulation), we might need to merge it here.
          // For now, assume backend returns correct "nav_actual".
          // "isPatched" logic relies on specific frontend rules which might need adjustment if logic moves to backend.
          // Current request says "Do not adjust frontend layout/features", implying we keep using patchRules if possible.
          // If backend returns simple history, we can still overlay patch rules if needed, 
          // OR we assume the backend data is what we display. 
          // Given "generateFundHistory" was replaced, we use API data directly.
          setHistoryData(data);
        }
      } catch (e) {
        console.error("Failed to fetch fund history", e);
      } finally {
...
const ClientDataLoader: React.FC<{
...
}> = ({ currentPortfolio, setPortfolio, refreshTrigger, children }) => {
  ...
  if (!currentPortfolio) {
    return <div className="text-center text-red-500 p-8">未找到该客户数据或加载失败</div>;
  }
          ...
          <div className="max-w-7xl mx-auto p-4 md:p-8">
            {loadingPortfolio ? (
              ...
              <Routes>
                ...
                <Route path="/portfolio" element={<ClientSearch targetPath="/portfolio" />} />
                <Route path="/portfolio/:clientId" element={
                  <ClientDataLoader currentPortfolio={portfolio} setPortfolio={setPortfolio} refreshTrigger={portfolioRefreshTrigger}>
                    <PortfolioPage portfolio={portfolio!} patchRules={patchRules} onAddExternalAsset={handleAddExternalAsset} onRefresh={() => setPortfolioRefreshTrigger(p => p + 1)} />
                  </ClientDataLoader>
                } />
                <Route path="/liquidity" element={<ClientSearch targetPath="/liquidity" />} />
                <Route path="/liquidity/:clientId" element={
                  <ClientDataLoader currentPortfolio={portfolio} setPortfolio={setPortfolio} refreshTrigger={portfolioRefreshTrigger}>
                    <LiquidityPage portfolio={portfolio!} funds={funds} updateHoldingRule={handleUpdateHoldingRule} updateAccountCash={handleUpdateAccountCash} />
                  </ClientDataLoader>
                } />
