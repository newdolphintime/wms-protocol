...
const LiquidityPage: React.FC<{ portfolio: ClientPortfolio, updateHoldingRule: (accId: string, holdingIdx: number, rule: RedemptionRule) => void, updateAccountCash: (accId: string, amount: number) => void }> = ({ portfolio, updateHoldingRule, updateAccountCash }) => {
  ...
  const projectionData = useMemo(() => { 
    ...
    for (let i = 0; i < days; i++) { 
        ...
        const dateStr = date.toISOString().split('T')[0]; const displayDate = `${date.getMonth()+1}-${date.getDate()}`; 
        ...
        dailyFlows.forEach(f => { if (f.relatedHoldingKey && f.type === 'INFLOW') { const currentRedeemed = redeemedAmounts.get(f.relatedHoldingKey) || 0; redeemedAmounts.set(f.relatedHoldingKey, currentRedeemed + f.amount); } }); 
